...
// do not include things that cannot go in the cart
    	$not_for_cart = $db->Execute("select p.products_id from " . TABLE_PRODUCTS . " p left join " . TABLE_PRODUCT_TYPES . " pt on p.products_type= pt.type_id where (pt.allow_add_to_cart = 'N') OR (p.manufacturers_id <> '" . $_SESSION['provider_id'] . "')");
