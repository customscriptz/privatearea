 ...
 * @version $Id: category_product_listing.php 17534 2010-09-08 19:50:34Z wilt $
 */
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="2">
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE; ?>&nbsp;-&nbsp;<?php echo zen_get_category_name($current_category_id, $_SESSION['languages_id']); ?></td>
...
?>
                ...
                <td class="dataTableContent" align="right">
                  <?php echo '<a href="' . zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $cPath . '&cID=' . $categories->fields['categories_id'] . '&action=edit_category') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT) . '</a>'; ?>
<?php
// bof: categories meta tags
		if ($_SESSION['allow_meta_tags'] == "1")
		{
...
<?php
    ...
    $products_count = 0;
	$status = ($_SESSION['provider_show_disabled_products'] == '0' ? " AND p.products_status = '1' " : '');
...
*/
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                       ...
                                       p.product_is_free, p.product_is_call, p.products_quantity_mixed, p.product_is_always_free_shipping,
                                       p.products_quantity_order_max, p.products_sort_order,
                                       p.master_categories_id, p.product_on_queue
                                from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, "
                                       . TABLE_PRODUCTS_TO_CATEGORIES . " p2c
                                where p.products_id = pd.products_id
                                and pd.language_id = '" . (int)$_SESSION['languages_id'] . "'
								and p.manufacturers_id = '" . (int)$_SESSION['provider_id'] . "'
                                and (p.products_id = p2c.products_id
      ...
      $products_query_raw = ("select p.products_type, p.products_id, pd.products_name, p.products_quantity,
                                       ...
                                       p.products_quantity_order_min, p.products_quantity_order_units, p.products_priced_by_attribute,
                                       p.product_is_free, p.product_is_call, p.products_quantity_mixed, p.product_is_always_free_shipping,
                                       p.products_quantity_order_max, p.products_sort_order, p.product_on_queue
...
if (($_GET['page'] == '1' or $_GET['page'] == '') and $_GET['pID'] != '') {
  ...
  if ($check_page->RecordCount() > $_SESSION['provider_products_per_page']) {
    $check_count=1;
    ...
    $_GET['page'] = round((($check_count/$_SESSION['provider_products_per_page'])+(fmod_round($check_count,$_SESSION['provider_products_per_page']) !=0 ? .5 : 0)),0);
    $page = $_GET['page'];
...
//      zen_redirect(zen_href_link(FILENAME_CATEGORIES, 'cPath=' . $_GET['cPath'] . '&pID=' . $_GET['pID'] . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '')));
    ...
    $products_split = new splitPageResults($_GET['page'], $_SESSION['provider_products_per_page'], $products_query_raw, $products_query_numrows);
...
// Split Page
	  ...
	  if ($products->fields['original_cat']) {
		echo '              <tr class="dataTableRow">' . "\n";
	  } else if (isset($pInfo) && is_object($pInfo) && ($products->fields['products_id'] == $pInfo->products_id)) {
...
?>
				...
				<td class="dataTableContent" width="50" align="left">
<?php
	  if ($products->fields['products_status'] == '0' && $products->fields['product_on_queue']) {
	  	echo zen_image(DIR_WS_IMAGES . 'icon_purple_on.gif', IMAGE_ICON_STATUS_QUEUE);
	  } else if ($products->fields['products_status'] == '1' AND !$products->fields['original_cat']) {
	  ...
	  } else if ($products->fields['products_status'] == '0' AND !$products->fields['original_cat']) {
...
<?php if ($products->fields['original_cat']) { ?>
				<td class="dataTableContent" align="right"><?php echo $products->fields['products_sort_order']; ?></td>
                <td class="dataTableContent" align="right">
        		<?php echo '<a href="' . zen_href_link($type_handler, 'action=move_product_confirm&products_id=' . $products->fields['products_id'] . '&do=restore') . '">' . zen_image(DIR_WS_IMAGES . 'icon_category_restore.gif', IMAGE_ICON_MOVED) . '</a>'; ?>
<?php } else if ($action == '') { ?>
                <td class="dataTableContent" align="right"><?php echo $products->fields['products_sort_order']; ?></td>
                <td class="dataTableContent" align="right">
        <?php echo '<a href="' . HTTP_SERVER  . DIR_WS_CATALOG . 'index.php?main_page=product_info&cPath=' . $cPath . '&products_id=' . $products->fields['products_id']  . '" target="_blank">' . zen_image(DIR_WS_IMAGES . 'icon_link.gif', '') . '</a>'; ?>
		<?php echo '<a href="' . zen_href_link($type_handler, 'cPath=' . $cPath . '&product_type=' . $products->fields['products_type'] . '&pID=' . $products->fields['products_id'] . '&action=new_product') . '">' . zen_image(DIR_WS_IMAGES . 'icon_edit.gif', ICON_EDIT) . '</a>'; ?>
	    ...
	    <?php if (file_exists('image_handler.php')) echo '<a href="' . zen_href_link(FILENAME_IMAGE_HANDLER, 'products_filter=' . $products->fields['products_id'] . '&current_category_id=' . $current_category_id) . '">' . zen_image(DIR_WS_IMAGES . 'icon_image_handler.gif', ICON_IMAGE_HANDLER) . '</a>'; ?>
		<?php if (file_exists('xsell_advanced.php')) echo '<a href="' . zen_href_link(FILENAME_XSELL_ADVANCED_PRODUCTS, 'model=' . $products->fields['products_model']) . '">' . zen_image(DIR_WS_IMAGES . 'icon_xsell.png', XSELL_PRODUCT) . '</a>'; ?>        
<?php
...
?>
<?php
        if ($zc_products->get_allow_add_to_cart($products->fields['products_id']) == "Y" AND $products->fields['specials_id'] > 0) {
          echo '<a href="' . zen_href_link(FILENAME_SPECIALS, 'sID=' . $products->fields['specials_id'] . '&action=edit') . '">' . zen_image(DIR_WS_IMAGES . 'icon_specials_edit.gif', ICON_PRODUCTS_EDIT_SPECIALS) . '</a>';
        }
		else if ($zc_products->get_allow_add_to_cart($products->fields['products_id']) == "Y") {
			echo '<a href="' . zen_href_link(FILENAME_SPECIALS, 'add_products_id=' . $products->fields['products_id'] . '&action=new') . '">' . zen_image(DIR_WS_IMAGES . 'icon_specials_add.gif', ICON_PRODUCTS_ADD_SPECIALS) . '</a>';
		}
// meta tags
		if ($_SESSION['allow_meta_tags'] == "1")
		{
...
<?php if ($action == '') { ?>
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText"><?php echo TEXT_PRODUCTS . '&nbsp;' . $products_count; ?></td>
                    <td align="right" class="smallText"><?php if (strpos($cPath_back, custom_get_provider_cpath()) !== false) echo '<a href="' . zen_href_link(FILENAME_CATEGORIES, $cPath_back . 'cID=' . $current_category_id) . '">' . zen_image_button('button_back.gif', IMAGE_BACK) . '</a>&nbsp;'; ?>
