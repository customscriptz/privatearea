 ...
 */
  ...
  if (isset($_POST['edit_x']) || isset($_POST['edit_y'])) {
    ...
    if ($_SESSION['provider_id']) $tmp_value = zen_db_prepare_input($_SESSION['provider_id']);
    ...
    $sql_data_array = array('products_quantity' => $products_quantity,
                            ...
                            'products_keywords' => zen_db_prepare_input($_POST['products_keywords'])
                            );
...
if ($_POST['image_delete'] == 1) {
    ...
    if ($action == 'insert_product') {
      ...
      if (PRODUCTS_ON_QUEUE == 'True') {
	  	$queue_sql_data = array( 'product_on_queue' => '1', 'products_status' => '0');
		$insert_sql_data = array_merge($insert_sql_data, $queue_sql_data);

		$subject = TEXT_SBJ_NEW_PRODUCT_ADDED . $_POST['products_name'][$_SESSION['languages_id']];
      	$message = sprintf(TEXT_MSG_NEW_PRODUCT_ADDED, STORE_OWNER, $_SESSION['provider_name'], $_POST['products_name'][$_SESSION['languages_id']]);
      	$messageHTML = sprintf(TEXT_MSG_NEW_PRODUCT_ADDED, '<strong>' . STORE_OWNER . '</strong>', '<strong>' . $_SESSION['provider_name'] . '</strong>', '<strong>' . $_POST['products_name'][$_SESSION['languages_id']] . '</strong>');

      	zen_mail(STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS, $subject, $message, STORE_NAME, EMAIL_FROM, array('EMAIL_MESSAGE_HTML'=>nl2br($messageHTML)), 'debug');
	...
	if ($action == 'insert_product')
	{
		zen_redirect(zen_href_link(FILENAME_PRODUCT, 'product_type=' . zen_db_prepare_input($_GET['product_type']) . '&cPath=' . $cPath . '&pID=' . $products_id . (isset($_GET['page']) ? '&page=' . $_GET['page'] : '') . (isset($_POST['search']) ? '&search=' . $_POST['search'] : '') . '&action=new_product&attributes=true'));
	}
