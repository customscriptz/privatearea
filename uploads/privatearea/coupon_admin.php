 ...
 */
  ...
  if ($_GET['action'] == 'voucheredit' && (int)$_GET['cid'])
  {	
	...
	{
		$messageStack->add_session(ERROR_CANNOT_EDIT);
    ...
    case 'confirmcopy':
      ...
      // create duplicate coupon
        $sql_data_array = array('coupon_code' => zen_db_prepare_input($coupon_copy_to),
                                ...
                                'manufacturers_id' => zen_db_prepare_input((int)$check_new_coupon->fields['manufacturers_id']),
...
//        zen_redirect(zen_href_link(FILENAME_COUPON_ADMIN));
    ...
    case 'update_confirm':
      if ( ($_POST['back_x']) || ($_POST['back_y']) ) {
        ...
        $sql_data_array = array('coupon_code' => zen_db_prepare_input($_POST['coupon_code']),
				...
				'manufacturers_id' => $_SESSION['provider_id']);
        $languages = zen_get_languages();
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
          $language_id = $languages[$i]['id'];
	...
	//configure the coupon to be restricted to the provider category AND denied to top category
	$sql = 'DELETE FROM ' . TABLE_COUPON_RESTRICT . ' WHERE coupon_id='.intval($_GET['cid']).' AND category_id IN (' . get_manufacturers_data($_SESSION['provider_id'], 'category_id') . ',"-1")';
	$db->Execute($sql);
	...
	$db->Execute("INSERT INTO " . TABLE_COUPON_RESTRICT . " (
	          	  ...
	          	  '" . intval($_GET['cid']) . "',
	          	  '" . get_manufacturers_data($_SESSION['provider_id'], 'category_id') . "',
	          	  'N'
	          	  )");
...
function check_form(form_name) {
  ...
  check_input('customers_email_address','',"<?php echo ERROR_NO_EMAIL_TO_SEND; ?>");
...
<?php
    $cc_query_raw = "select * from " . TABLE_COUPON_REDEEM_TRACK . " c1 LEFT JOIN " . TABLE_COUPONS . " c2 ON (c1.coupon_id = c2.coupon_id) where c1.coupon_id = '" . $_GET['cid'] . "' and c2.manufacturers_id = '" .  $_SESSION['provider_id']. "'";
...
?>
              <tr>
                ...
                <td><?php echo zen_draw_input_field('customers_email_address') . zen_draw_hidden_field('from', EMAIL_FROM);?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
              </tr>
...
?>
              <tr>
                ...
                <td><?php echo zen_draw_input_field('subject', DEFAULT_SBJ, 'size="50"'); ?></td>
              </tr>
              <tr>
                <td colspan="2"><?php echo zen_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
              </tr>
...
<?php
    if ($status != '*') {
	 $cc_query_raw = "select coupon_id, coupon_code, coupon_amount, coupon_type, coupon_start_date,coupon_expire_date,uses_per_user,uses_per_coupon,restrict_to_products, restrict_to_categories, date_created,date_modified, coupon_active, coupon_zone_restriction from " . TABLE_COUPONS ." where manufacturers_id = '" . $_SESSION['provider_id'] . "' AND coupon_active='" . zen_db_input($status) . "' and coupon_type != 'G'";
    } else {
      $cc_query_raw = "select coupon_id, coupon_code, coupon_amount, coupon_type, coupon_start_date,coupon_expire_date,uses_per_user,uses_per_coupon,restrict_to_products, restrict_to_categories, date_created,date_modified, coupon_active, coupon_zone_restriction from " . TABLE_COUPONS . " where manufacturers_id = '" . $_SESSION['provider_id'] . "' AND coupon_type != 'G'";
	}
    
    if ($status == 'Y')
	$cc_query_raw .= ' AND coupon_expire_date >= now()';
    if ($status == 'N')
	$cc_query_raw .= " OR (manufacturers_id = '" . $_SESSION['provider_id'] . "' AND coupon_expire_date <= now())";
