 ...
 */
if (!isset($_SESSION['html_editor_preference_status_privatearea'])) {
	$_SESSION['html_editor_preference_status_privatearea'] = HTML_EDITOR_PREFERENCE;
}
/**
 ...
 *
 * $_SESSION['html_editor_preference_status_privatearea'] = the key name of the selected editor for this session
...
foreach($editors_pulldown as $key=>$value) {
	if ($new_editor_choice == $value['id']) $_SESSION['html_editor_preference_status_privatearea'] = $value['key'];
	if ($_SESSION['html_editor_preference_status_privatearea'] == $value['key']) $current_editor_key = $value['id'];
}
$editor_handler = DIR_WS_INCLUDES . $editors_list[$_SESSION['html_editor_preference_status_privatearea']]['handler'];
...
if ($editor_handler != '' && !file_exists($editor_handler)) {
	$editor_handler = '';
	$_SESSION['html_editor_preference_status_privatearea'] = 'NONE';
...
if (false) {
	...
	echo '<br />$_SESSION[html_editor_preference_status_privatearea] = ' . $_SESSION['html_editor_preference_status_privatearea'];
