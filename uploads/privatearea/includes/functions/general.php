...
function zen_get_category_tree_special($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $category_has_products = false, $limit = false) {
	global $db;
	...
	$provider_cPath = $_SESSION['provider_cpath'];
	...
	while (!$categories->EOF) {
		...
		$category_cPath = custom_get_provider_cpath($categories->fields['categories_id']);

		if($category_cPath == $provider_cPath || strpos($category_cPath, $provider_cPath . '_') === 0)
		{
			if ($exclude != $categories->fields['categories_id']) $category_tree_array[] = array('id' => $categories->fields['categories_id'], 'text' => $spacing . $categories->fields['categories_name'] . $mark);
		}
			$category_tree_array = zen_get_category_tree_special($categories->fields['categories_id'], $spacing . '&nbsp;&nbsp;&nbsp;', $exclude, $category_tree_array, '', $category_has_products);
		$categories->MoveNext();
	}
