...
<?php
  ...
  $provider_cPath = custom_get_provider_cpath(get_manufacturers_data($_SESSION['provider_id'], 'category_id'));
  ...
  while (!$prev_sales->EOF) {
    ...
    foreach($categories_array as $category) {
	  ...
	  if(strpos($category['path'], $provider_cPath . '_') === 0)
	  {
	  	echo '        <tr valign="top"><td><table border="0" cellspacing="2" cellpadding="2">' . "\n";
        echo '      <tr onClick="RowClick(\'' . $category['path'] . '\')">' . "\n";
		echo '        <td width="10">' . zen_draw_checkbox_field('categories[]', $category['path'], 'true') . "</td>\n";
	  	echo '        <td width="10"></td>';
...
<?php
    $salemaker_sales_query_raw = "select sale_id, sale_status, sale_name, sale_deduction_value, sale_deduction_type, sale_pricerange_from, sale_pricerange_to, sale_specials_condition, sale_categories_selected, sale_categories_all, sale_date_start, sale_date_end, sale_date_added, sale_date_last_modified, sale_date_status_change from " . TABLE_SALEMAKER_SALES . " where manufacturers_id = '" . $_SESSION['provider_id'] . "' order by sale_name";
