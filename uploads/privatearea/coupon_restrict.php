 ...
 */
  define('MAX_DISPLAY_RESTRICT_ENTRIES', 100);
  require('includes/application_top.php');

  checkAccess('coupon_restrict', true);
  
  $canedit = true;
  if ((int)$_GET['cid'])
  {
      $countuse = $db->Execute("select * from " . TABLE_COUPON_REDEEM_TRACK . "
                                       where coupon_id = '" . $_GET['cid'] . "'");
	if ($countuse->RecordCount())
	{
		$messageStack->add(ERROR_CANNOT_EDIT);
		$canedit = false;
	}
  }
...
<?php
		if ($cr_list->fields['coupon_restrict']=='N') {
			if ($cr_list->fields['category_id'] != "-1")
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ALLOW) . '</td>';
			...
			if ($cr_list->fields['category_id'] != "-1")
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ALLOW) . '</td>';
				//echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'action=switch_status&info=' . $cr_list->fields['restrict_id'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ALLOW) . '</a></td>';
			}
			else
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ALLOW) . '</td>';
		...
		if ($cr_list->fields['coupon_restrict']=='Y') {
			if ($cr_list->fields['category_id'] != "-1")
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_DENY) . '</td>';
				//echo '<td class="dataTableContent" align="center"><a href="' . zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'action=switch_status&info=' . $cr_list->fields['restrict_id'], 'NONSSL') . '">' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_DENY) . '</a></td>';
			}
			else
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_DENY) . '</td>';
			...
			if ($cr_list->fields['category_id'] != "-1")
			{
				echo '<td class="dataTableContent" align="center">' . zen_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_DENY) . '</td>';
...
?>
				...
				<td class="dataTableContent" align="left"><?php echo '<strong>' . $product_name . '</strong><br />' . TEXT_CATEGORY . zen_get_categories_name_from_product($pr_list->fields['product_id']); ?></td>
<?php
	if ($canedit)
	{
...
?>
              ...
              <tr><?php if ($canedit) { ?><form name="restrict_category" method="post" action="<?php echo zen_href_link('coupon_restrict.php', zen_get_all_get_params(array('info', 'action', 'x', 'y')) . 'action=add_category&info=' . $cInfo->restrict_id, 'NONSSL'); ?>"><?php echo zen_draw_hidden_field('securityToken', $_SESSION['securityToken']); ?>
                <td colspan="7"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
<?php
...
?>
                    ...
                    <td class="smallText" align="left"><?php echo zen_draw_pull_down_menu('cPath_prod', zen_get_category_tree_special(), $selected_category_id, 'onChange="this.form.submit();"'); ?></td></form>
